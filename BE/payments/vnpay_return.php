...
$app_redirect_url = "onefood://payment_result";
        ...
        try {
            ...
            if ($current_status === 'HoanThanh') {
                ...
                if ($result_check_promo && $result_check_promo->num_rows > 0) {
                    ...
                    $promotion_id = intval($promo_data['promo_id']);
                    error_log("Found promotion. Discount amount: " . $discount_amount . ", Promo ID: " . $promotion_id);
                    $final_amount = $amount;
                }
                ...
                $payment_method = trim('ChuyenKhoan');
                ...
                if ($promotion_id != null) {
                    $sql_decrement_promo = "UPDATE promotions SET promo_quantity = promo_quantity - 1 WHERE promo_id = ? AND promo_quantity > 0";
                    $stmt_decrement_promo = $conn->prepare($sql_decrement_promo);
                    $stmt_decrement_promo->bind_param("i", $promotion_id);
                    $stmt_decrement_promo->execute();
                    $affected_rows = $stmt_decrement_promo->affected_rows;
                    error_log("Decremented promotion quantity. Promo ID: " . $promotion_id . ", Affected rows: " . $affected_rows);
                    $stmt_decrement_promo->close();
                }
            }
